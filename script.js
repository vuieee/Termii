...
const unixImages = [
    ...
    { src: "assets/images/arch-rice.png", name: "Arch Rice" }
];
    ...
    if (loadedCount >= allImages.length) {
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                document.querySelectorAll('.article-thumb img').forEach((img, idx) => { if(articlesData[idx]) img.src = articlesData[idx].img; });
            }, 500);
        }, 200);
    }
}
...
function openSocials() { closeAllOverlays(null, socialsContainer); populateSocials(); socialsContainer.style.display = 'flex'; requestAnimationFrame(() => { socialsContainer.classList.add('active'); mainStage.classList.add('blur'); }); }
...
// --- REPOS ---
function openRepos() { closeAllOverlays(null, reposContainer); renderRepos(); reposContainer.style.display = 'flex'; requestAnimationFrame(() => { reposContainer.classList.add('active'); mainStage.classList.add('blur'); }); }
function renderRepos() {
    const container = document.getElementById('repos-list-content'); container.innerHTML = '';
...
function openUnix() {
    ...
    unixImages.forEach(imgData => {
        if(imgData.name.toLowerCase().includes('rice')) {
            const thumb = document.createElement('div');
            thumb.className = 'unix-thumb';
            thumb.innerHTML = `<img src="${imgData.src}">`;
            thumb.onclick = () => viewUnixImage(imgData.src);
...
function updateImageTransform() { document.getElementById('unix-expanded-img').style.transform = `scale(${currentZoom}) translate(${translateX}px, ${translateY}px)`; }
...
function updateBiosStats() {
    ...
    document.getElementById('val-tasks').innerText = rnd(100, 200);
}
